...
const Rect terrain_buttons_rect = {0,0,550,210};
const Rect terrain_rect_gr_size = {0,0,512,512};
const Rect base_small_button_from = {103,0,113,10};
const Rect base_small_3D_button_from = {143,0,154,16};
...
void Get_right_sbar_rect( Rect * rect )
{
	...
	rect->bottom = 22 * (TER_BUTTON_SIZE + 1) + 1;
...
//	    map_pat[i] = GetPixPat (200 + i);
	...
	editor_mixed = load_pict(915);//was 906
...
void set_up_terrain_buttons()
{
	...
	Rect ter_from,ter_from_base = {0,0,16,16};
 	...
 	paint_pattern(terrain_buttons_gworld,0,terrain_buttons_rect,3);
	SetPort((GrafPtr) terrain_buttons_gworld);
				...
				switch (current_drawing_mode) {
					...
					case 1: case 2:
						...
						if (sbar_pos * 12 + i < 512) {
							...
							if(
(store_ter_type >= 2 && store_ter_type <= 73)
||
(
			...
			if (do_this_item == TRUE) {
				...
				if (index < 0) {
					if (current_drawing_mode == 0)
						...
						else cant_draw_graphics_error(a,"Error was for terrain type",store_ter_type);
					return;	
					}
...
Boolean place_terrain_icon_into_ter_large(graphic_id_type icon,short in_square_x,short in_square_y)
{
	...
	rect_draw_some_item(graphics_library[index],
		from_rect,ter_draw_gworld,to_rect,1,0);
	return TRUE;
}
...
{
	Rect ter_script_icon_from = {143,160,157,183};
...
{
	Rect ter_script_icon_from = {103,100,123,120};
					  ...
					  (town.creatures[i].start_loc.x == q) && (town.creatures[i].start_loc.y == r)) {
						...
						if (town.creatures[i].start_attitude < 3)
							put_rect_in_gworld(ter_draw_gworld,to_rect,0,255,0);
								...
								else put_rect_in_gworld(ter_draw_gworld,to_rect,0,0,255);
...
void place_right_buttons(short mode)
{
	...
	//SetPort((GrafPtr) terrain_buttons_gworld);
	for (short i = 0; i < 5; i++) {
 		paint_pattern(terrain_buttons_gworld,0,right_text_lines[i],3);
		//FillCRect(&right_text_lines[i],bg[6]);
		}
	//SetPort(mainPtr);
	
	if (file_is_loaded == FALSE) {
		//sprintf((char *) draw_str,"No campaign loaded.");
		//char_win_draw_string((GrafPtr) terrain_buttons_gworld,right_text_lines[0],(char *) draw_str,2,12);
	...
	/*
	if (editing_town) {
		sprintf((char *) draw_str,"Town: %d  %s ",cur_town,town.town_name);
		char_win_draw_string((GrafPtr) terrain_buttons_gworld,right_text_lines[0],(char *) draw_str,2,12);
		}
		else {
			sprintf((char *) draw_str,"Outdoor: X=%d Y=%d  %s ",cur_out.x,cur_out.y,current_terrain.name);
			char_win_draw_string((GrafPtr) terrain_buttons_gworld,right_text_lines[0],(char *) draw_str,2,12);
			}
			
	sprintf((char *) draw_str,"Center: x = %d, y = %d ",cen_x,cen_y);
	char_win_draw_string((GrafPtr) terrain_buttons_gworld,right_text_lines[1],(char *) draw_str,2,12);
	*/
	...
	/*if (current_drawing_mode == 0) {
		sprintf((char *) draw_str,"Floor: %s (%d)",
			scen_data.scen_floors[current_floor_drawn].floor_name,current_floor_drawn);
		char_win_draw_string((GrafPtr) terrain_buttons_gworld,right_text_lines[2],
		  draw_str,2,12);
		}
	if (current_drawing_mode == 1) {
		sprintf((char *) draw_str,"Terrain: %s (%d)",
			scen_data.scen_ter_types[current_terrain_drawn].ter_name,current_terrain_drawn);
		char_win_draw_string((GrafPtr) terrain_buttons_gworld,right_text_lines[2],
		  draw_str,2,12);
		}*/
	if (current_drawing_mode == 2) {
		if (current_height_mode == 0)
			...
			else char_win_draw_string((GrafPtr) terrain_buttons_gworld,right_text_lines[2],
			  "Automatic Hills: ON",2,12);
		}
		
	char_win_draw_string((GrafPtr) terrain_buttons_gworld,right_text_lines[3],(char *) current_string,2,12);
	char_win_draw_string((GrafPtr) terrain_buttons_gworld,right_text_lines[4],(char *) current_string2,2,12);
	
	//to_rect = whole_palette_from;
	//OffsetRect(&to_rect,PALETTE_BUT_UL_X,PALETTE_BUT_UL_Y);
	//rect_draw_some_item(editor_mixed,whole_palette_from,
		//terrain_buttons_gworld,to_rect,0,0);
	...
	for (short i = 0; i < 8; i++)
		for (short j = 0; j < ((editing_town == TRUE) ? 6 : 3); j++) {
			...
			if(cur_viewing_mode >= 10 && i == 0 && j == 1)
